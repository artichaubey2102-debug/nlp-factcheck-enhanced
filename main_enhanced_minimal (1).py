...
with st.sidebar:
    ...
    # NEW: API Key Input
    st.header("🔑 Google Fact Check API (Optional)")
    ...
    api_key = st.text_input("Enter API Key", type="password", help="Optional: Enables Google Fact Check comparison")
    
    if api_key and api_key.strip():
        st.success("✅ API Key provided")
    else:
        st.info("💡 Skipping Google Fact Check")
    ...
    if st.button("🚀 Run Model Comparison"):
        ...
        if api_key and api_key.strip():
            ...
            with st.spinner("Fetching fact check data from Google API..."):
                ...
                google_labels = get_google_fact_check_labels(X.tolist(), test_indices, api_key, progress_bar)
            ...
            st.info("🔑 Enter a Google Fact Check API key in the sidebar to enable comparison with external fact-checking data.")
